package TestData;// File: CreateSeaPortDataFile.java
// Date: Jul 8, 2016
...
public class CreateSeaPortDataFile {
   ...
   static int nd = 5; // number of docks per port + 5 (>0)
   static int np = 13; // number of passenger ships per port
   static int nc = 7; // number of cargo ships per port
   static int nj = 3; // number of jobs per dock (>0)
   static int nn = 10; // number of persons per port + 5
   static String filename = ".\\PopulatedData\\test4.txt";
...
class MyThing implements Comparable <MyThing> {
   ...
   void readSkillsFile () {
      try {
         ...
         Scanner sp = new Scanner (new File ("./ReadFiles/skillNames.txt"));
...
class MySeaPort extends MyThing {
   ...
   void readPortsFile () {
      try {
         ...
         Scanner sp = new Scanner (new File ("./ReadFiles/portNames.txt"));
...
class MyShip extends MyThing {
   ...
   void readWordsFile () {
      try {
         words = new ArrayList <> ();
         Scanner sp = new Scanner (new File ("./ReadFiles/shipNames.txt"));
...
class MyPerson extends MyThing {
   ...
   void readNamesFile () {
      try {
         words = new ArrayList <> ();
         Scanner sp = new Scanner (new File ("./ReadFiles/personNames.txt"));
         while (sp.hasNext()) words.add (sp.next());
